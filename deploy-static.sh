#!/bin/bash

echo "🚀 Автоматический деплой на Vercel..."

# Проверяем, установлен ли Vercel CLI
if ! command -v vercel &> /dev/null; then
    echo "❌ Vercel CLI не установлен. Устанавливаем..."
    npm install -g vercel
fi

# Подготавливаем статические файлы
echo "📦 Подготовка статических файлов..."
./deploy-static.sh

# Создаем папку public и копируем файлы
echo "📁 Подготовка структуры для Vercel..."
mkdir -p public
cp dist/*.html dist/*.css dist/*.js public/ 2>/dev/null || true
cp -r dist/admin public/ 2>/dev/null || true

# Проверяем авторизацию в Vercel
echo "🔐 Проверяем авторизацию в Vercel..."
if ! vercel whoami &> /dev/null; then
    echo "❌ Не авторизованы в Vercel. Выполните: vercel login"
    exit 1
fi

# Деплоим на Vercel
echo "🌐 Деплоим на Vercel..."
vercel --prod --yes

echo "✅ Деплой завершен!"
echo ""
echo "🔗 Ваш сайт доступен по адресу: https://online-school-online-school-nextjs.vercel.app"
echo ""
echo "📋 Не забудьте:"
echo "1. Проверить работу всех страниц"
echo "2. Протестировать регистрацию и вход"
echo ""
echo "📊 Мониторинг: https://vercel.com/dashboard"
